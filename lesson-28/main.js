...
let AscIdBtn = document.querySelector(".ascId");
let DescIdBtn = document.querySelector(".descId");
let AscNameBtn = document.querySelector(".ascName");
let DescNameBtn = document.querySelector(".descName");
...
function darwTable(item) {
  ...
  item.forEach((element) => {
    ...
    trElem.innerHTML += `<td>${element.id}</td>
        <td>${element.companyName}
        </td><td>${element.contactName}
        </td><td>${element.address.country}
        </td><td>${element.address.phone}</td>
        </td><td>${element.address.postalCode}</td>
        `;
    tBody.append(trElem);
  });
}
...
AscIdBtn.addEventListener("click", function () {
  let sorted = suppliers.sort((a, b) => a.id - b.id);
  darwTable(sorted);
});

DescIdBtn.addEventListener("click", function () {
  let sorted = suppliers.sort((a, b) => b.id - a.id);
  darwTable(sorted);
});

AscNameBtn.addEventListener("click", function () {
  let sorted = suppliers.sort((a, b) => {
...
DescNameBtn.addEventListener("click", function () {
  let sorted = suppliers.sort((a, b) => {
    if (a.companyName.toLocaleLowerCase() > b.companyName.toLocaleLowerCase())
      return -1;
    if (a.companyName.toLocaleLowerCase() < b.companyName.toLocaleLowerCase())
